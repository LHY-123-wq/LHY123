...
def set_chinese_font():
    # 直接指定常用的中文字体名称列表，确保在Windows环境下能找到
    preferred_fonts = ['Microsoft YaHei', 'SimHei', 'KaiTi', 'FangSong', 'STSong', 'STHeiti', 'DejaVu Sans']
    ...
    if selected_font:
        ...
        print("Warning: No Chinese fonts found. Using default font.")
...
if not company_data.empty:
    # 方案1：使用Streamlit内置的line_chart
    # 准备数据
    chart_data = company_data[['年份', '数字化转型指数']].set_index('年份')
    
    # 使用Streamlit内置的折线图（推荐用于Web环境，中文显示更可靠）
    st.line_chart(chart_data, height=600, width=1200)
    
    # 方案2：使用Matplotlib但优化字体设置
    # 创建Matplotlib图表作为备选
    ...
    # 优化字体设置，使用Web安全字体方案
    plt.rcParams['svg.fonttype'] = 'none'  # 保持字体为矢量格式
    plt.rcParams['font.sans-serif'] = ['Arial Unicode MS', 'DejaVu Sans']  # 使用Unicode字体
    plt.rcParams['font.family'] = 'sans-serif'
    ...
    # 在图表下方添加图例说明
    st.write(f"当前选择: {selected_stock_code_str} - {company_name} ({selected_year}年)")
    st.write(f"当前指数: {current_index:.2f}" if not filtered_data.empty else "无数据")
    
    # 可选：显示Matplotlib图表（用于对比）
    # st.pyplot(fig)
else:
