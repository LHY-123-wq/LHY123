...
def set_chinese_font():
    ...
    if os.path.exists(yahei_path):
        try:
            # 强制添加字体到Matplotlib
            fm.fontManager.addfont(yahei_path)
            print(f"已添加微软雅黑字体: {yahei_path}")
    ...
    # 检查是否成功添加了字体
    yahei_found = any('msyh' in font.fname.lower() for font in fm.fontManager.ttflist)
    print(f"系统中是否存在微软雅黑字体: {yahei_found}")
...
if not company_data.empty:
    ...
    ax.set_xticks(years)
    # 显式设置x轴刻度标签字体
    ax.set_xticklabels(years, rotation=0, fontsize=10, fontname='Microsoft YaHei')
    # 显式设置y轴刻度标签字体
